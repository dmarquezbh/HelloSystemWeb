// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
using System;
using System.Collections.Generic;

#nullable disable

namespace DADOS.Models
{
    public partial class CategoriaDespesaUnidadeOrganizacional
    {
        public int IdCategoriaDespesa { get; set; }
        public int IdUnidadeOrganizacional { get; set; }
        public DateTime DtLancamento { get; set; }
        public double NrPercentualDesconto { get; set; }
        public int IdUsuario { get; set; }
        public bool InAtual { get; set; }
        public bool? InAtivo { get; set; }

        public virtual TipoDiversos IdCategoriaDespesaNavigation { get; set; }
        public virtual UnidadeOrganizacional IdUnidadeOrganizacionalNavigation { get; set; }
        public virtual Usuario IdUsuarioNavigation { get; set; }
    }
}